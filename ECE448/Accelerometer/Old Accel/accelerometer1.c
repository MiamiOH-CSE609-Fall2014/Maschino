...
void tapHandler()
{
	int source = readRegister(PULSE_SRC);  // Reads the PULSE_SRC register
...
{
	int pl = readRegister(0x10);  // Reads the PL_STATUS register
...
void motionDetect(){
	int source = readRegister(FF_MT_SRC);
...
// Read i registers sequentially, starting at address into the dest int array
...
// Read a single int from address and return it as a int
